...
data "tls_certificate" "cert" {
  url = aws_eks_cluster.it.identity[0].oidc[0].issuer
}
...
module "oidc-provider-data" {
  ...
  version    = "0.0.3"
  issuer_url = aws_eks_cluster.it.identity[0].oidc[0].issuer
}

# For nodes
resource "aws_iam_role" "it-noda" {
...
resource "aws_iam_role_policy_attachment" "kubeedge-node-AmazonSecretsPolicy" {
  ...
  role       = aws_iam_role.it-noda.name
}
